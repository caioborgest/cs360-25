...
import { InternalTeamManagement } from './InternalTeamManagement';
import { 
  Handshake, 
  TrendingUp, 
  DollarSign, 
...
export const PartnersManagement = () => {
  ...
  const partnerStats = [
    {
      ...
      value: partners.filter(p => p.status === 'approved').length.toString(),
      change: "+12",
      icon: Handshake,
      color: "text-blue-600"
    },
    {
      title: "Leads Gerados",
      value: partners.reduce((sum, p) => sum + p.leadsGenerated, 0).toLocaleString(),
      change: "+234",
      icon: Users,
  ...
  const handleNewPartner = (partnerData: any) => {
    const newPartner: Partner = {
      id: `prt_${Date.now()}`,
      name: partnerData.companyName,
      type: partnerData.partnershipType,
    ...
    console.log('Novo parceiro criado:', partnerData);
  };
                        ...
                        <TableCell>
                          <div className="text-sm">
                            ...
                            <div className="text-green-600">
                              {partner.leadsGenerated > 0 ? ((partner.conversions / partner.leadsGenerated) * 100).toFixed(1) : '0'}% conv.
                            </div>
                          </div>
                        </TableCell>
