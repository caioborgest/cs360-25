...
const availableIntegrations = [
  ...
  { name: 'Asana', category: 'Gestão', logo: '✅', color: 'from-pink-500 to-red-500', description: 'Colaboração e tarefas' },
  { name: 'WhatsApp Business', category: 'Comunicação', logo: '💚', color: 'from-green-500 to-green-600', description: 'Comunicação via WhatsApp' },
  { name: 'Oracle', category: 'Database', logo: '🔴', color: 'from-red-600 to-red-700', description: 'Banco de dados empresarial' },
  { name: 'AWS', category: 'Cloud', logo: '☁️', color: 'from-orange-500 to-yellow-500', description: 'Serviços em nuvem' },
  { name: 'Google Cloud', category: 'Cloud', logo: '🌐', color: 'from-blue-500 to-green-500', description: 'Plataforma Google Cloud' }
];
...
export const IntegrationsAPI = () => {
  ...
  const filteredAvailableIntegrations = selectedIntegrationCategory === 'Todos' 
    ? availableIntegrations 
    : availableIntegrations.filter(integration => integration.category === selectedIntegrationCategory);

  const getStatusBadge = (status: string) => {
    const configs = {
      active: { label: 'Ativo', className: 'bg-green-100 text-green-800', icon: CheckCircle },
      inactive: { label: 'Inativo', className: 'bg-gray-100 text-gray-800', icon: Clock },
      error: { label: 'Erro', className: 'bg-red-100 text-red-800', icon: AlertTriangle },
      revoked: { label: 'Revogada', className: 'bg-red-100 text-red-800', icon: XCircle }
    };
        ...
        <CardHeader className="pb-6">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {categories.map((category) => (
              <button
                key={category}
                onClick={() => setSelectedIntegrationCategory(category)}
                className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 ${
                  selectedIntegrationCategory === category
